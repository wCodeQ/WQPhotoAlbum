...
class WQPhotoAlbumLisetViewController: WQPhotoBaseViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private func getAllAlbum() {
        ...
        fetchResult.enumerateObjects({ [unowned self] (assetCollection, index, nil) in
            ...
            guard assetsFetchResult.count <= 0 else {
                ...
                if assetCollection.localizedTitle == "所有照片" || assetCollection.localizedTitle == "All Photos" {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var albumsCell: WQAlbumCell? = tableView.dequeueReusableCell(withIdentifier: "AlbumsCell") as? WQAlbumCell
        if albumsCell == nil {
            albumsCell = WQAlbumCell(style: .default, reuseIdentifier: "AlbumsCell")
        }
...
private class WQAlbumCell: UITableViewCell {
