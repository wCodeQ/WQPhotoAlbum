//
//  WQPhotoClipViewController.swift
//  WQPhotoAlbumKit
//
//  Created by 王前 on 2017/4/27.
//  Copyright © 2017年 qian.com. All rights reserved.
//

import UIKit

class WQPhotoClipViewController: WQPhotoBaseViewController, UIScrollViewDelegate {
    
    var clipBounds: CGSize = CGSize(width: WQScreenWidth, height: WQScreenWidth) {
        didSet {
            self.photoImageScrollView.frame = CGRect(origin: CGPoint(x: (WQScreenWidth-clipBounds.width)/2, y: (WQScreenHeight-clipBounds.height)/2), size: clipBounds)
        }
    }
    ...
    private lazy var photoImageScrollView: UIScrollView = {
        ...
        var frame = CGRect(origin: CGPoint(x: 0, y: (WQScreenHeight-WQScreenWidth)/2), size: self.clipBounds)
    ...
    private lazy var maskView: WQPhotoClipMaskView = {
        let view = WQPhotoClipMaskView(frame: self.view.frame, clipBounds: self.clipBounds)
        return view
    }()
    ...
    var photoImage: UIImage? {
        didSet {
            ...
            if WQScreenWidth > clipBounds.width {
                self.photoImageScrollView.contentOffset.x = (WQScreenWidth-clipBounds.width)/2
            } else {
    ...
    func scrollViewDidZoom(_ scrollView: UIScrollView) {
        var xcenter = scrollView.center.x-(WQScreenWidth-clipBounds.width)/2 , ycenter = scrollView.center.y-(WQScreenHeight-clipBounds.height)/2
...
fileprivate class WQPhotoClipMaskView: UIView {
    ...
    convenience override init(frame: CGRect) {
        self.init(frame: frame, clipBounds: CGSize(width: WQScreenWidth, height: WQScreenWidth))
    }
    ...
    init(frame: CGRect, clipBounds: CGSize) {
        ...
        path.append(UIBezierPath(rect: CGRect(origin: CGPoint(x: (WQScreenWidth-clipBounds.width)/2, y: (WQScreenHeight-clipBounds.height)/2), size: clipBounds)).reversing())
